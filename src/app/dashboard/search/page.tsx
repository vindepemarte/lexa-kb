      ...
      <main className="relative z-10 max-w-4xl mx-auto px-4 py-8 sm:py-12">
        ...
        {searched && (
          <div className="space-y-4">
              ...
              <p className="text-white/70 text-sm">
                {results.length === 0 ? (
                  ...
                  <span className="font-medium text-purple-300">
                    ...
                    {isLimited && <span className="text-white/40"> (showing {results.length})</span>}
                  </span>
                )}
              </p>
              {results.length > 0 && (
            ...
            {isLimited && (
              <div className="mt-6 bg-gradient-to-r from-purple-600/20 to-pink-600/20 backdrop-blur-xl border border-purple-500/30 rounded-2xl p-6 text-center">
                <Lock className="w-6 h-6 text-pink-400 mx-auto mb-3" />
                <p className="text-white font-semibold mb-1">
                  {totalCount - results.length} more result{totalCount - results.length !== 1 ? 's' : ''} available
                </p>
                <p className="text-white/60 text-sm mb-4">Upgrade to Personal to see all search results</p>
                <a
                  href="/dashboard/pricing"
                  className="inline-block px-6 py-2.5 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white font-semibold rounded-xl shadow-lg transition-all text-sm"
                >
                  Upgrade for €9/mo
                </a>
              </div>
